...
class SentenceEncoder(nn.Module):
    ...
    def forward(self,x):
        ...
        x = torch.cat(x,dim=1)
        x = x.view(self.config.BATCH_SIZE,-1,400)
...
class DocumentEncoder(nn.Module):
    ...
    def __init__(self,config=None):
        ...
        if config is not None:
            self.config = config
        self.gru_layer = nn.GRU(input_size=400,hidden_size=400,num_layers=1,batch_first=True,bidirectional=True)
    ...
    def forward(self,x):
        ...
        Shapes 
        X = (N,num_sentences, sentence_features[=400]) 400 is the features for the sentence here
        D_i => (N,hidden_size*2[since it is bidirectional])
        H_i = (N,num_sentences,hidden_size)
        
        Returns
        D_i,output(ie,H_i)
        '''
        output,h_n = self.gru_layer(x)
        D_i = torch.tanh(h_n.view(self.config.BATCH_SIZE,-1))
